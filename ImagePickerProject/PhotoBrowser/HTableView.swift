...
protocol HTableViewCellProtocol: NSObjectProtocol {
  var reuseIdentifier: String {get set}
}

protocol HTableViewDataSource: NSObjectProtocol{
...
@objc protocol HTableViewDelegate: NSObjectProtocol{
  ...
  optional func hTableView(hTableView: HTableView, widthForColumnAtIndex index: Int) -> CGFloat
  optional func hTableView(hTableView: HTableView, didSelectRowAtIndex: Int)
  optional func hTableViewDidScroll(hTableView: HTableView)
  optional func hTableViewWillBeginDragging(hTableView: HTableView)
  optional func hTableViewDidEndDecelerating(hTableView: HTableView)
  optional func hTableViewWillEndDragging(hTableView: HTableView)
  optional func hTableViewDidEndDragging(hTableView: HTableView, willDecelerate decelerate: Bool)
...
class HTableView: UIScrollView {
  
  //  private var scrollView: UIScrollView!
  var dataSource: HTableViewDataSource!{
    didSet{
      loadData()
    }
  }
  var delegateForHTableView: HTableViewDelegate?
  ...
  //可见的所有cell
  private var visibleCellList: [UITableViewCell] = []
  //可供复用的cell
  private var reusableCellListDic = Dictionary<String, [UITableViewCell]>()
  //储存上次一发生滑动的可见区域
  ...
  private func loadData(){
    ...
    for index in 0..<numberOfColumns {
      ...
      if delegateForHTableView != nil && delegateForHTableView!.respondsToSelector("hTableView:widthForColumnAtIndex:"){
        cellWidth = delegateForHTableView!.hTableView!(self, widthForColumnAtIndex: index)
      } else {
  ...
  private func addCellIntoHTableView(index: Int, direction: RollDirection){
    ...
    let cell = dataSource.hTableView(self, cellForColumnAtIndex: index)
    cell.frame = cellRect
    cell.tag = index
    addSubview(cell)
  ...
  func didSelectedCell(cell: UITableViewCell){
    let currentSelectedIndex = cell.tag
    delegateForHTableView?.hTableView?(self, didSelectRowAtIndex: currentSelectedIndex)
  ...
  func cellForRowAtIndex(index: Int) -> UITableViewCell {
...
extension HTableView: UIScrollViewDelegate {
  
  func scrollViewDidScroll(scrollView: UIScrollView) {
    
    delegateForHTableView?.hTableViewDidScroll?(self)
    
    let visibleRect = self.visibleRect()
    let offset = CGRectGetMinX(visibleRect) - CGRectGetMinX(perviousVisibleRect)
    
    //当一次移动的距离超过本身的可视宽度时则重新添加cell
    if offset > CGRectGetWidth(frame) {
  ...
  func scrollViewWillBeginDragging(scrollView: UIScrollView) {
    delegateForHTableView?.hTableViewWillBeginDragging?(self)
  }
  
  func scrollViewWillEndDragging(scrollView: UIScrollView, withVelocity velocity: CGPoint, targetContentOffset: UnsafeMutablePointer<CGPoint>) {
    delegateForHTableView?.hTableViewWillEndDragging?(self)
  }
  
  func scrollViewDidEndDecelerating(scrollView: UIScrollView) {
    delegateForHTableView?.hTableViewDidEndDecelerating?(self)
  }
  
  func scrollViewDidEndDragging(scrollView: UIScrollView, willDecelerate decelerate: Bool) {
    delegateForHTableView?.hTableViewDidEndDragging?(self, willDecelerate: decelerate)
  }
